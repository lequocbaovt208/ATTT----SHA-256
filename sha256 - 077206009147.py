# SHA - 256 : Le Quoc Bao - 077206009147
import sys
import io
...
def sha256_hash(message):
    ...
    while (len(message_bytes) * 8) % 512 != 448:
        message_bytes += b'\x00'
    message_bytes += msg_bit_len.to_bytes(8, 'big')
    hash_values = H.copy()
    ...
    hash_hex = ''.join(f'{value:08x}' for value in hash_values)
    return hash_hex
def main():
    print("="*70)
    print("THUẬT TOÁN SHA-256 - HÀM BĂM")
    print("="*70)
    message = "LeQuocBao2812006"
    print(f"\nInput: {message}")
    hash_result = sha256_hash(message)
    print(f"Output: {hash_result}")  
    print("\n" + "="*70)
    print("\nMột số ví dụ khác:")
    test_messages = ["HELLO", "A",]
